...
angular.module('joynRideApp').controller('MainController', function ($scope, $window, Request, ngTableParams, NotifyService) {
    ...
    $scope.submitRide = function () { //driver presses submit button
        ...
        Request.get('/insert_transaction?user_id=' + JSON.parse(localStorage.user).user_id + '&src=' + $scope.map.from.name + '&srcx=' + $scope.map.from.geometry.location.A + '&srcy=' + $scope.map.from.geometry.location.F + '&dst=' + $scope.map.to.name + '&dstx=' + $scope.map.to.geometry.location.A + '&dsty=' + $scope.map.to.geometry.location.F + '&time=' + extractTime($scope.ride.time) + '&date=' + extractDate($scope.ride.date) + '&price=' + $scope.ride.price||0 + '&free_sits=' + $scope.ride.maxPlaces||4 + '&radius=' + $scope.ride.radius||0 + '&path=' + getPath()+'&msg='+$scope.ride.msg||"-"
            , function (data) {
